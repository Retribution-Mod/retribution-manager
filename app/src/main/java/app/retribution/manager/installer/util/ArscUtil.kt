...
package app.retribution.manager.installer.utils
...
object ArscUtil {
    ...
    /**
     * Adds a new drawable resource pointing to a file inside the APK.
     *
     * @param name The new resource name.
     * @param path The path to the drawable inside the APK (e.g. `res/drawable/icon.png`).
     * @return The resource ID of the newly added drawable.
     */
    fun BinaryResourceFile.addDrawableResource(
        name: String,
        path: String,
    ): BinaryResourceIdentifier {
        val pathIdx = this.getMainArscChunk().stringPool.addString(path)
        return this.getPackageChunk().addResource(
            typeName = "drawable",
            resourceName = name,
            configurations = { true },
